...
def cpe_conversion(cpe):
    """
    This function converts a CPE 2.2 string into the CPE 2.3 format

    Parameters:
      - cpe (str): The 2.2 cpe string to be converted.
    """

    # If already in 2.3 format, do nothing
    if cpe.startswith("cpe:2.3:"):
        return cpe
    
    # Remove "cpe:/" or "cpe:" prefix
    if cpe.startswith("cpe:/"):
        cpe = cpe[5:]  # removes "cpe:/"
    ...
    # The CPE 2.3 format contains 13 components (including "cpe" and "2.3")
    # format is cpe : 2.3 : part : vendor : product : version : update : edition : lang : sw_ed : tgt_sw : tgt_hw : other
    # Therefore, 11 fields should follow "cpe:2.3:"
    while len(parts) < 11:
        parts.append("*")
...
def cve_scan(ip, details_map):
    ...
    for port_protocol, service_data in services.items():
        ...
        for cpe_string in cpe_list:
            ...
            url = f"https://services.nvd.nist.gov/rest/json/cves/2.0?cpeName={cpe_converted}"
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if vulnerabilities_list:
                        print(f"      Detected {len(vulnerabilities_list)} vulnerabilities for '{cpe_string}' ({port_protocol})!")
                        ...
                        print(f"      No CVE detected for '{cpe_string}' ({port_protocol}).")
...
def save_results():
    ...
    os.chmod(output_scan, 0o777) # Grant full permissions to the output file
